...
section.section
  .container
    label.label Enter track related text
    .field.has-addons
      .control
        input.input(v-model="query", type='text')
      //- p.help This is a help text
      .control
        button.button.is-primary(@click="searchGalibooTracks") Submit
  hr
...
export default {
  ...
  methods: {
    async searchGalibooTracks() {
      await this.$store.dispatch('searchGalibooTracks', this.query)
    }
  }
}
</script>
